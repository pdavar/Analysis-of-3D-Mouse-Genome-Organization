...
"""
def pckmeans_get_dist_mat_binned(cell_data_mat, bins, num_bins_per_chr): #returns a 19 x 19 distance matrix: THE DIAGONALS WILL NOT BE 0
    num_bins = len(bins)- 1
    ...
    groups = cell_data_mat.groupby([pd.cut(cell_data_mat.abs_pos, bins), pd.cut(cell_data_mat.pckmeans_cluster, [-0.1,0.9,2])]).mean()                          
